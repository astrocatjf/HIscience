...
def train_unet(params, out_dir):
    ...
    val_generator = dataloaders.dataLoaderDeep21(
        ...
        # create checkpoint method to save model in event of crash
        ## LATER: modify to compute 2D power spectrum
        best_fname = out_dir + 'best_model_%d.h5'%int(params['model_num'])
        model_checkpoint = ModelCheckpoint(best_fname, monitor='val_mse', verbose=0,
                                            save_best_only=True, mode='auto', save_freq='epoch')
        best_fname = out_dir + 'best_weights_%d.h5'%int(params['model_num'])
    ...
    if __name__ == '__main__':
        ...
        if not os.path.exists(out_dir): 
            print("Path does not exist. Making directory...")
            os.mkdir(out_dir)
