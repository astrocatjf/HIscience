...
config_file_path = '/home/rubin/deep21/configs/configs_deep21.json'
...
print("NU_START: {}".format(NU_ARR[0]))
print("NU_STOP: {}".format(NU_ARR[-1]))
...
if __name__ == '__main__':
    ...
    print('NU_ARR[0]: ', NU_ARR[0])
    print('NU_ARR[-1]: ', NU_ARR[-1])

    for _ in np.arange(1,NUM_SIMS + 1):
        # Open the Fits files for foreground and cosmological signal
        # fgd_names = ["%s/gsync/gsync_%03d_001.fits"%(dirstr,nu) for nu in NU_ARR]
        # fgd_names = fgd_names[::NU_AVG]
        # print(len(fgd_names), fgd_names)
        # cosmo_names = ["%s/cosmo/cosmo_%03d_001.fits"%(dirstr,nu) for nu in NU_ARR]
        # cosmo_names = cosmo_names[::NU_AVG]
        # print(len(cosmo_names), cosmo_names)
        print(["%s/fg2/%03d/fg_%03d.fits"%(dirstr,SNUM,nu+1) for nu in NU_ARR])
        print(["%s/cosmo2/%03d/cosmo_%03d.fits"%(dirstr,SNUM,nu+1) for nu in NU_ARR])
        # fgd = np.array([h5py.File("%s/fg/fg_%03d.hdf5"%(dirstr,nu),"r")['map'][0,:] for nu in NU_ARR],dtype=np.float64).T
        fgd = np.array([fits.getdata("%s/fg2/%03d/fg_%03d.fits"%(dirstr,SNUM,nu+1)) for nu in NU_ARR],dtype=np.float64).T
        # fgd = np.array([fits.getdata(name) for name in fgd_names],dtype=np.float64).T
        # cosmo = np.array([h5py.File("%s/cosmo/cosmo_%03d.hdf5"%(dirstr,nu),"r")['map'][0,:] for nu in NU_ARR],dtype=np.float64).T
        cosmo = np.array([fits.getdata("%s/cosmo2/%03d/cosmo_%03d.fits"%(dirstr,SNUM,nu+1),1) for nu in NU_ARR],dtype=np.float64).T
        # cosmo = np.array([fits.getdata(name) for name in cosmo_names],dtype=np.float64).T
        print('fgd shape: ', fgd.shape) 
        ...
        print("DO_NU_AVG: ", DO_NU_AVG)
        if DO_NU_AVG:
            # average in frequency bins and transpose
            print('fgd shape before avg: ', fgd.shape)
            print('cosmo shape before avg: ', cosmo.shape)
            print('cosmo_n shape before avg: ', cosmo_n.shape)
            ...
            print('fgd shape after avg: ', fgd.shape)
            print('cosmo shape after avg: ', cosmo.shape)
            print('cosmo_n shape after avg: ', cosmo_n.shape)

        else:
            # skip every NU_AVG frequency 
            print('cosmo shape before skip: ', cosmo.shape)
            fgd = fgd.T[::NU_AVG].T
            cosmo = cosmo.T[::NU_AVG].T
            cosmo_n = cosmo_n.T[::NU_AVG].T
            print('cosmo shape after skip: ', cosmo.shape)
        ...
        print("obs_pca.shape: ", obs_pca.shape)
        ...
        for s,N_COMP_MASK in enumerate(COMPONENTS):
            ...
            print("%s/pca%d_sim%03d"%(out_dir,N_COMP_MASK,SNUM))            
        ...
        for s,sig in enumerate([cosmo, obs]):
            ...
            print("%s/%s_sim%03d"%(out_dir,out_names[s],SNUM))
